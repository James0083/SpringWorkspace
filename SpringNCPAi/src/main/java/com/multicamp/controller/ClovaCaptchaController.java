...
public class ClovaCaptchaController {
	...
	public String getKeyCode() {
		try {
			...
			return key;
			/*
			 * 캡차 키 발급 요청시 응답 ---------------------------- key string {"key":"acd12"}
			 * 
			 */
	...
	public void catchImage(HttpServletResponse res) {
		try {
			this.key = this.getKeyCode();// https://naveropenapi.apigw.ntruss.com/captcha/v1/nkey 호출로 받은 키값
			...
			if (responseCode == 200) { // 정상 호출
				...
				f.createNewFile();
				// OutputStream outputStream = new FileOutputStream(f); //원본코드 수정. 파일에 이미지 데이터를
				// 출럭[x]
				// 우리는 파일에 내보내는 것이 아니라 브라우저에 이미지 데이터를 출력하자

				OutputStream outputStream = res.getOutputStream();// 브라우저와 연결된 출력 스트림을 얻어와서 해당 스트림으로 이미지 데이터를 출력
	...
	public ModelAndView captchaCheck(@RequestParam("userInput") String userInput) {
