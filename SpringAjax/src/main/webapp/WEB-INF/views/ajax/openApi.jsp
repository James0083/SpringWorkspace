...
<script type="text/javascript">
	...
	function showPage(total, keyword, display, cpage){
		...
		//}
		//페이지수
		//pageCount=(total-1)/display +1 <==java
		//let pageCount=Math.floor((total-1)/display +1); //<== 자바스크립트 실수점
	...
	function test(){
		//자바스크립트 cors 정책에 위배되어 같은 도메인이 아닌 곳에서 네트워크 통신이 불가능
		// ==> 대신 자바로 네이머에 요청을 보내보자. NaverBookProxy.java
		...
		$.ajax({
			...
			cache:false,
			headers:{
				'X-Naver-Client-Id':'CZw9TdJMAzytVdJjc3JH',
				'X-Naver-Client-Secret':'wnF0dEr0I5' 
			}
		})
		.done((res)=>{
			alert(res)
		...
		.fail((err)=>{
			alert('error: '+err.status+", message: "+err.responseText)
		})
	}
