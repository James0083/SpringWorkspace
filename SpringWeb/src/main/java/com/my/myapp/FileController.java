...
public class FileController {
	...
	public String fileUpload2(Model m, HttpServletRequest req) {
		...
		if(fList!=null) {
			for(int i=0;i<fList.size();i++) {
				...
				//실제 업로드 되는 파일명
				String fname=uidStr+"_"+mf.getOriginalFilename(); //uuid_filename.png
				//사용자가 업로드한 원본파일명
				String origin = mf.getOriginalFilename();
				
				//동일한 파일명을 업로드시 나중에 업로드한파일이 이전 파일을 덮어쓰기하는 문제
	 ...
	 * */
	//produces="application/octet-stream" : 브라우저가 열 수 있는 형식 설정 ,  application/octet-stream : 응용프로그램(.exe, .java, ...)
	...
	public ResponseEntity<org.springframework.core.io.Resource> fileDownload(
		...
		if(checkIE) {
			...
			//그 외 브라우저인 경우
			origin_fname=origin_fname.replace(",","");//크롬은 파일명에 콤마(,)가 있으면 다운로드가 되지 않음
		...
		//3. HttpHeader통해 헤더 벙보 설정해서 ResponseEntity에 담아 반환함
