...
//functions/SMO ==> 서포트 벡터머신 알고리즘 (주의. 데이터가 정규화 또는 표준화가 되어있어야 함)
public class Weka05SupportVectorMachine {
	String file="/Users/jaewonyou/Applications/weka/weka-3-9-3/data/Titanic/titanic_ko_remove_reorder_normalize.arff";
	...
	public void loadArff(String file) throws Exception {
		...
		//==> reorder했으므로 마지막 속성을 정답 데이터로 지정한다.
	...
	public void generateModel_Evaluate() throws Exception{
		...
		System.out.println("---------------------------");
		System.out.printf("전체 데이터 건수: %d%n", (int)eval.numInstances());
		System.out.printf("정 분류 건수: %d%n", (int)eval.correct());
