...
public class IntStack {
	...
	private int ptr;//배열의 인덱스로 사용되는 포인터. push()할 때는 ptr이 증가, pop()할 때는 감소
	...
	/**데이터를 삽입. 스택(배열)이 가드가서 추가할 수 없을 때 예외 발생 */
	public int push(int val) throws Exception{
		if(ptr>=capacity) throw new Exception("OverflowException");
		return stk[ptr++]=val;
	}
	
	/** 데이터 꺼내기 : 스택에서 꼭대기에 있는 데이터를 꺼내 반환. 포인터는 감소 시킴 */
	public int pop() throws Exception{
		if(ptr<=0) throw new Exception("EmptyException");
		return stk[--ptr];
	}
