...
public class BinaryTree<K,V> {
	...
	private int compare(K key1, K key2) {
		if(this.comparator!=null) {
			...
			//String,Integer 클래스들은 Comparable인터페이스를 상속받고 있다.
	...
	private void addNode(Node<K, V> node, K key, V data) {
		...
		if(cond==0) {//key가 이진트리에 이미 있는 경우
			return;
		}else if(cond<0){//새로 추가할 노드가 부모노드값 보다 작다면 된쪽 방향 진행
			if(node.left==null) {
	...
	public V search(K key) {
		...
		while(true) {
			...
			if(cond==0) {
				...
				p=p.left;//왼쪽 서브트리에서 계속 검색
			}else {
				p=p.right;//오른쪽 서브트리에서 계속 검색
			}
	...
	public boolean remove(K key) {
		...
		while(true) {
			...
			if(cond==0) break;//삭제할 노드를 찾았다면 반복문 벗어나기
			else {
				parent=p;
				if(cond<0) {
					isLeftChild=true;
		...
		if(p.left==null) {//검색한 p노드의 왼쪽에 자식이 없다면
			...
			//삭제할 노드의 자식 노드가 둘 다 있을 경우 ==> 왼쪽 서브트리 중에서 가장 큰 노드를 찾는다
			parent=p;//5
